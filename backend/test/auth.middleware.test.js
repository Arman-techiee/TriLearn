...
test('protect falls back to legacy user lookup when Prisma client lacks passwordChangedAt/deletedAt', async () => {
  const findUnique = async ({ where }) => {
    if (Object.prototype.hasOwnProperty.call(where, 'deletedAt')) {
      throw new Error('Unknown field `passwordChangedAt` for select statement on model `User`.')
    }
