...
const HomePage = () => (
  <div className="min-h-screen bg-[linear-gradient(180deg,#f3efe6_0%,#fffdf8_45%,#eef7f3_100%)] text-slate-900">
    <header className="border-b border-slate-200/70 bg-[--color-bg-card] dark:bg-slate-800/70 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
        <div>
          <p className="text-2xl font-black tracking-tight text-primary">TriLearn</p>
          <p className="text-xs uppercase tracking-[0.28em] text-slate-500">Academic Management Platform</p>
        </div>
        <nav className="flex items-center gap-3">
          <Link
            to="/login"
            className="rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white transition hover:bg-primary-700"
          >
            Sign In
          </Link>
        </nav>
    ...
    <main>
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.15fr_0.85fr] lg:px-10 lg:py-24">
        <div className="max-w-3xl">
          <p className="mb-4 inline-flex rounded-full border border-primary-200 bg-primary-50 px-4 py-1 text-sm font-semibold text-primary">
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
            TriLearn helps schools and colleges bring academic coordination, classroom workflows, and student access
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-3">
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-3xl border border-white/70 bg-[--color-bg-card] dark:bg-slate-800/80 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
        ...
        <div className="relative">
          ...
          <div className="relative rounded-[2rem] border border-white/30 bg-slate-950/90 p-8 text-white shadow-[0_30px_80px_rgba(15,23,42,0.18)]">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-primary-200">Inside TriLearn</p>
            <div className="mt-6 space-y-5">
              <div className="rounded-2xl bg-[--color-bg-card] dark:bg-slate-800/8 p-5">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-6 lg:px-10">
        <div className="grid gap-6 lg:grid-cols-3">
          {highlights.map((item) => (
            <article key={item.title} className="rounded-[1.75rem] border border-slate-200 bg-[--color-bg-card] dark:bg-slate-800/85 p-7 shadow-[0_20px_50px_rgba(15,23,42,0.05)]">
        ...
        <div className="rounded-[2rem] bg-slate-900 px-8 py-10 text-white shadow-[0_28px_80px_rgba(15,23,42,0.16)]">
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-2xl">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-accent-300">Who It Serves</p>
              <h2 className="mt-4 text-3xl font-black tracking-tight">A platform shaped around real campus roles.</h2>
              <p className="mt-4 text-sm leading-7 text-slate-300">
                TriLearn is designed so each part of the institution can move faster without losing oversight. The result is
                less friction for staff and better visibility for students.
              </p>
            </div>
            <div className="grid flex-1 gap-4">
              {roleCards.map((card) => (
                <div key={card.role} className="rounded-2xl border border-white/10 bg-[--color-bg-card] dark:bg-slate-800/6 p-5">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 lg:px-10">
        <div className="rounded-[2rem] border border-primary-200 bg-[linear-gradient(135deg,#ecfdf5_0%,#eff6ff_100%)] p-8 shadow-[0_22px_60px_rgba(16,185,129,0.08)] sm:p-10">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                to="/login"
                className="rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white transition hover:bg-primary-700"
              >
                Go to Sign In
              </Link>
            </div>
