    ...
    <InstructorLayout>
      <div className="p-8">
        ...
        <PageHeader
          ...
          breadcrumbs={['Instructor', 'Routine']}
        />
              ...
              {DAYS.map(day => (
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition whitespace-nowrap ${
                    activeDay === day ? 'bg-green-600 text-white' : 'bg-white text-gray-600 border hover:bg-gray-50'
                  }`}
                >
                  {DAY_SHORT[day]}
                </button>
              ...
              {DAYS.map(day => (
                <div
                  ...
                  className={`rounded-2xl border p-4 shadow-sm ${
                    day === todayName() ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-white'
                  } ${activeDay === day ? 'ring-2 ring-emerald-200' : ''}`}
                >
                  <div className="mb-4 flex items-center justify-between">
                    <div>
                      <p className={`text-sm font-bold ${day === todayName() ? 'text-emerald-700' : 'text-slate-700'}`}>{DAY_SHORT[day]}</p>
                      <p className="text-xs text-slate-400">{byDay[day].length} classes</p>
                    </div>
                    {day === todayName() ? <span className="ui-status-badge ui-status-success">Today</span> : null}
                  </div>
                      ...
                      byDay[day].map((r) => (
                        <div key={r.id} className={`rounded-2xl border p-4 ${subjectColorMap[r.subjectId]}`}>
                          ...
                          <p className="mt-2 text-xs text-gray-500">
                            {r.department || r.subject?.department || 'General'} • Semester {r.semester}{r.section ? ` • Section ${r.section}` : ' • All sections'}
                          </p>
                          {r.room ? <p className="mt-2 text-xs text-gray-500">Room: {r.room}</p> : null}
                        </div>
                      ))
                    )}
                  </div>
